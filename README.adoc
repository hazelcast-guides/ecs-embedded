:github-address: https://github.com/hazelcast-guides/hazelcast-embedded-ecs
...
For the next steps, let's assume that we always use the VPC with ID `vpc-0681043d6f49b039a` (CIDR `vpc-10.0.0.0/16`). For the sake of simplicity, let's put it as an env variable:

----
export VPN_ID='vpc-0681043d6f49b039a'
----

To include `hazelcast.yaml` inside your Spring Boot project, copy it into `hazelcast-embedded-springboot/src/resources/`.
...
By default, Hazelcast uses the port 5701 for all the communication. Therefore, we need to create an appropriate security group.

----
$ aws ec2 create-security-group --group-name hazelcast-security-group --description "Hazelcast Security Group" --vpc-id ${VPC_ID}
{
    "GroupId": "sg-0cf1116753b96cdce"
}
----

Let's store the security group id as an environment variable.
...
Hazelcast ECS Discovery uses AWS API to discover Hazelcast members. That is why we need to grant it certain permissions. Here are the commands to create an IAM Role we will later use by ECS Tasks.
...
sed -i "s/REGION/eu-central-1/g" task-definition.json
----
...
Finally, when all set and done we can start ECS Service which in turn will create ECS Tasks. First, choose the subnet in which our service should operate.

----
$ aws ec2 describe-subnets --filter --filters "Name=vpc-id,Values=${VPC_ID}" --query 'Subnets[*].[SubnetId,CidrBlock]'
[
    [
...
You can check that the related tasks were created.
...
You can also examinte the logs of one of the tasks and you should see that Hazelcast cluster was successfully formed.
...
$ aws ecs describe-tasks --cluster test-cluster \
  --tasks 2154b675-df19-459e-a95e-8466f5d4bb59 7c3adb1d-6850-4f86-9eb1-7eed5bc75235 99de6977-949d-489c-9c5a-44c763edfaaf \
...
$ aws ec2 describe-network-interfaces \
  --network-interface-ids eni-0050d027099aeb812 eni-061b0e2414cfb1440 eni-0464651cd2cdf1b45 \
...
----
aws ecs update-service --cluster test-cluster --service hazelcast-embedded --desired-count 0
aws ecs delete-service --cluster test-cluster --service hazelcast-embedded
----
